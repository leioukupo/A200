...
enum {
	...
	,IMI_DEPTH_IR_COLOR_FRAME     = 0x0A  //USBLITE 支持三图同出 20210301
};
...
typedef uint32_t ImiDeviceAnalzinParamsStatus;
 enum
{
	IMI_PARAMS_ANALZIN_OK								 = 0x00,   // 从文件读取并解析正确
	IMI_PARAMS_ANALZIN_FILE_INFO_CRC_NOT_CORRECT		 = 0x01,   //文件内容和文件内容计算出的crc不匹配
	IMI_PARAMS_ANALZIN_FILE_SN_NOT_CORRECT				 = 0x02,   //解析文件SN不匹配
	IMI_PARAMS_ANALZIN_FILE_FW_VERSION_NOT_CORRECT       = 0x03,   //解析文件固件版本号不匹配
	IMI_PARAMS_ANALZIN_FILE_SDK_VERSION_NOT_CORRECT      = 0x04,   //解析文件SDK版本号不匹配
	IMI_PARAMS_ANALZIN_FILE_DEPTHPARAMS_NOT_CORRECT      = 0x05,   //解析文件深度参数不匹配
	IMI_PARAMS_ANALZIN_FILE_SIZE_NOT_CORRECT		     = 0x06,   //文件长度不正确
	IMI_PARAMS_ANALZIN_FILE_NOT_EXIST		             = 0x07,   //文件不存在
	IMI_PARAMS_OPEN_FILE_FAILED		                     = 0x08,   //打开文件失败
};

typedef uint32_t ImiDeviceReadParamsStatus;
 enum
{
	IMI_PARAMS_READ_OK									 = 0x00,   // 从设备读取参数正确
	IMI_PARAMS_NOT_READ_FROM_DEVICE						 = 0x01,   // 从文件读取且文件解析正确，未从设备读取
	IMI_PARAMS_READ_CRC_NOT_CORRECT						 = 0x02,   //从设备读取深度参数不正确
	IMI_PARAMS_READ_SN_NOT_CORRECT						 = 0x03,   //从设备读取SN不正确
	IMI_PARAMS_READ_FW_VERSION_NOT_CORRECT               = 0x04,   //从设备读取固件版本号不正确
	IMI_PARAMS_READ_DEPTHPARAMS_NOT_CORRECT              = 0x05,   //从设备读取深度参数不正确
};
