...
int main(int argc, char** argv)
{
	...
	//4.imiSetFrameMode()
    const ImiFrameMode* pFrameMode = imiGetCurrentFrameMode(g_ImiDevice, IMI_COLOR_FRAME);
    ...
    imiSetFrameMode(g_ImiDevice, IMI_COLOR_FRAME, (ImiFrameMode*)pFrameMode);

	//5.imiOpenStream()
	ret = imiOpenStream(g_ImiDevice, IMI_COLOR_FRAME, nullptr, nullptr, &g_streams[g_streamNum++]);
	if(0 != ret)
	{
    ...
    g_pRender = new SampleRender("RGB Color View", pFrameMode->resolutionX, pFrameMode->resolutionY);  // window title & size
