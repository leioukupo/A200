...
| ImiFramiStreamHandle* | pStream   | 用于保存 Stream 句柄的内存地址 |   OUT   |
...
> 1. 调用 imiInitialize2，指定驱动类型为 FILE，指定文件路径或修改配置文件 imi.ini,确认[Common]Driver=Dummy，然后配置Data文件路径， [File_Driver] Record_File_Path="c:/1.data" 
...
```c++
int32_t  imiTakePhoto (const char* pBsmpImagePath)
```

|      Type       |  Name  |           Description            | IN/OUT |
|:---------------:|:------:|:--------------------------------:|:------:|
| const char* | pBsmpImagePath |  指向图片保存路径的指针 |   IN   |
