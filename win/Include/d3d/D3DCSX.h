...
DECLARE_INTERFACE_(ID3DX11FFT, IUnknown) {
    ...
    0,D3DX11_FFT_MAX_TEMP_BUFFERS)
    UINT NumTempBuffers,
    __in_ecount(NumTempBuffers) ID3D11UnorderedAccessView
    *const* ppTempBuffers,
    __in_range(0,D3DX11_FFT_MAX_PRECOMPUTE_BUFFERS)
    UINT NumPrecomputeBuffers,
    __in_ecount(NumPrecomputeBuffers) ID3D11UnorderedAccessView
    *const* ppPrecomputeBufferSizes ) PURE;
    ...
    STDMETHOD(ForwardTransform)(THIS_
                                __in const ID3D11UnorderedAccessView

    * pInputBuffer,
    __inout ID3D11UnorderedAccessView **ppOutputBuffer
    ) PURE;
