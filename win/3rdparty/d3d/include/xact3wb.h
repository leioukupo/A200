...
#define ADPCM_MINIWAVEFORMAT_BLOCKALIGN_CONVERSION_OFFSET 22
    ...
    DWORD EncodeWMABlockAlign(DWORD dwBlockAlign, DWORD dwAvgBytesPerSec) const {
        ...
        for (; dwBlockAlignIndex < MAX_WMA_BLOCK_ALIGN_ENTRIES &&
               dwBlockAlign != aWMABlockAlign[dwBlockAlignIndex]; dwBlockAlignIndex++);

        if (dwBlockAlignIndex < MAX_WMA_BLOCK_ALIGN_ENTRIES) {
            for (; dwBytesPerSecIndex < MAX_WMA_AVG_BYTES_PER_SEC_ENTRIES &&
                   dwAvgBytesPerSec != aWMAAvgBytesPerSec[dwBytesPerSecIndex]; dwBytesPerSecIndex++);
    ...
    void AdpcmFillCoefficientTable(ADPCMWAVEFORMAT *fmt) const {
        ...
        static ADPCMCOEFSET aCoef[7] = {{256, 0},
                                        {512, -256},
                                        {0,   0},
                                        {192, 64},
                                        {240, 0},
...
typedef struct WAVEBANKENTRYCOMPACT {
    ...
    void SwapBytes(void) {
        XACTWaveBank::SwapBytes(*(LPDWORD)
        this);
    }
...
inline void
WAVEBANKMINIWAVEFORMAT::XMA2FillFormatEx(XMA2WAVEFORMATEX *fmt, WORD blockCount, const WAVEBANKENTRY *entry) const {
